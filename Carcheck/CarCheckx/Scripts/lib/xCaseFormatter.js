...
{
	//alert('caseFormateer');
  _TextBoxFormats = new xTextBoxFormatters(); 
}
...
function xTextBoxFormatters()
{
  ...
  this.AddTextBoxToWatch = function(str_textbox_id) {  
    if ($(str_textbox_id).className.indexOf("ENTERO") > 0)
    {
      xAddEventListener($(str_textbox_id), 'keyPress', onlyNumbers, true);
      xAddEventListener($(str_textbox_id), 'blur', formatAsNumberEntero, false);	  
    }
    else
    {
      if ($(str_textbox_id).className.indexOf("REAL") > 0)
      {
        xAddEventListener($(str_textbox_id), 'keyPress', onlyNumbers, true);
        xAddEventListener($(str_textbox_id), 'blur', formatAsNumberReal, false);
      }
      else
      {
        xAddEventListener($(str_textbox_id), 'blur', evaluateCase, true);
      }
    }  
  }
