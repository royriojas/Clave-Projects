...
CCSOL.DOM = {
  ...
  MakeClear : function(img,flag){
	...
	xShowLoadingMessage: function (mensaje,top,right,padding,color) {	
		if (!mensaje) { 
			mensaje = ':: Procesando ::'; 
		}
		if (!right){
	    ...
	    if ($('xxxdivLoadingMessage') == null) {
	        ...
	        xAppendChild(document.body,divLoadingMessage);
			    ...
			    $('xxxdivLoadingMessage').innerHTML = mensaje;			    
			    $('xxxdivLoadingMessage').style.top = top;
			    $('xxxdivLoadingMessage').style.right = right;
			    $('xxxdivLoadingMessage').style.padding = padding;
			    $('xxxdivLoadingMessage').style.backgroundColor = color;			    
	        $('xxxdivLoadingMessage').style.display = 'block';
	    }
